...
class PsychoPyCustomDisplay(pylink.EyeLinkCustomDisplay):
    ...
    def __init__(self, window, tracker):
        ...
        # Build beeps with fallback — PTB audio backend can crash if a Window
        # is already open when Sound() is called.
        def _make_sound(value, secs, octave):
            try:
                return psychopy.sound.Sound(value=value, secs=secs, octave=octave)
            except Exception:
                return None

        self.beeps = {
            pylink.CAL_TARG_BEEP: _make_sound('C', 0.2, 5),
            pylink.DC_TARG_BEEP:  _make_sound('C', 0.2, 5),
            pylink.CAL_GOOD_BEEP: _make_sound('A', 0.2, 6),
            pylink.DC_GOOD_BEEP:  _make_sound('A', 0.2, 6),
            pylink.CAL_ERR_BEEP:  _make_sound('E', 0.5, 4),
            pylink.DC_ERR_BEEP:   _make_sound('E', 0.5, 4),
        }
        ...
        self.image_title_object = psychopy.visual.TextStim(
            self.window, text='', pos=(0, -200), height=10, units='pix', color=self.text_color
        )
