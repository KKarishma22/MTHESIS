...
def EyeLinker(window, filename, eye, text_color=None):
    ...
    if response == 'q':
        window.flip()
        raise e if e is not None else RuntimeError("EyeLink: refused")
    elif response == 'd':
        window.flip()
...
class ConnectedEyeLinker:
    ...
    def send_tracking_settings(self, settings=None):
        ...
        # elcl_select_configuration is only valid for RS-232 connections, not ethernet (CL mode)
        try:
        ...
        # binocular_enabled MUST be set before active_eye, otherwise
        # the tracker rejects active_eye while still in binocular mode.
        if self.eye == 'BOTH':
        ...
        self.send_command('saccade_motion_threshold = %s' % settings['saccade_motion_threshold'])
